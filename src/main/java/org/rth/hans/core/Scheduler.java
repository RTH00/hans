...
public class Scheduler extends Thread {
    ...
    public static final boolean INITIAL_JOB_IS_ACTIVATED = false;
    ...
    /*** For running jobs ***/
    private final LinkedList<StartedExecution> startedExecutions;
    ...
    public Scheduler(final JobParser jobParser,
        ...
        this.startedExecutions = new LinkedList<>();
    }
    ...
    public boolean runLoop(final long maxIterations) {
        try {
            ...
            // TODO allow to skip inital configuration & default values
    ...
    // TODO add partition when a job is not activated?
    ...
    private boolean addNewPartitionsInExecutionLogs() throws SQLException {
        ...
        if(next != null && now.toEpochMilli() > next.getActivationThreshold().toEpochMilli()) {
            ...
            /*
            if(next.getActivationThreshold() ??? ) {
                // TODO don't add a waiting partition if the job is not activated
                // no impact on the logic but on perf
            }
            */
            // add in database
            ...
            if(next.getActivationThreshold().compareTo(job.getEndPartition()) < 0) {
                waitingPartitions.add(
                        new WaitingPartition(
                                next.getJobName(),
                                nextStartPartition,
    ...
    private void updateRunningExecutors() throws SQLException {
        final Iterator<StartedExecution> it = startedExecutions.iterator();
    ...
    private boolean addRunningExecutor(final int maxExecutors) throws SQLException, IOException {
        if(startedExecutions.size() >= maxExecutors) {
            // max already reached
            return false;
        }
        ...
        startedExecutions.add(startExecution(job, partition, processBuilder, stderrFile));
        return true;
    }
