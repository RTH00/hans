...
class DatabaseTests {
    ...
    @Test
    void updateJobIsActivatedTest() throws Exception {
        try(final TemporaryDatabase db = new TemporaryDatabase("temp_database_update_job_is_activated.db")) {
            db.pushConfiguration(new JobConfiguration(
                    8,
                    new Duration(1234, ChronoUnit.MILLIS),
                    new Duration(2345, ChronoUnit.MILLIS),
                    new Job[]{new Job(
                            ...
                            Utils.parseSqliteFormat("2000-01-01 00:00:00"),
                            Utils.parseSqliteFormat("2000-01-01 00:00:00"),
                            new Duration(3600, ChronoUnit.SECONDS),
                            new String[]{"/bin/echo", "toto"},
                            new Dependency[0],
                            1,
                            Job.FailureBehavior.RETRY,
                            ...
                            "/dev/null",
                            "/dev/null",
                            Scheduler.INITIAL_JOB_IS_ACTIVATED
                    )})
            );

            Assertions.assertFalse(db.getJob("jobNameTest").getIsActivated());
            db.updateJobIsActivated("jobNameTest", true);
            Assertions.assertTrue(db.getJob("jobNameTest").getIsActivated());
            db.updateJobIsActivated("otherName", false);
            Assertions.assertTrue(db.getJob("jobNameTest").getIsActivated());
            db.updateJobIsActivated("jobNameTest", false);
            Assertions.assertFalse(db.getJob("jobNameTest").getIsActivated());
        }
    }
